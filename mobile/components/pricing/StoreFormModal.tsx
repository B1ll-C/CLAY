...
import { storeInputSchema } from "@clay/shared";

interface StoreFormModalProps {
  visible: boolean;
  onClose: () => void;
  /** Present for edit; absent for a new store. */
  initial?: Store;
  /** Called with the new store's id after a successful create. */
  onCreated?: (id: number) => void;
}

/** Create or edit a store — mirrors `ListFormModal`'s 2-field shape. */
export function StoreFormModal({
  ...
  function handleSave() {
    ...
    if (isEdit) {
      ...
      createStore.mutate(parsed.data, {
        onSuccess: (id) => {
          onClose();
          onCreated?.(id);
        },
      ...
      >
        <View className="w-full rounded-3xl bg-white p-5">
          ...
          <TextInput
            ...
            placeholder="e.g. Countdown Newmarket"
            value={name}
