...
> - **`feat/background-workers`'s `CleanupWorker.purgeOldSoftDeletes` purges children before parents** (`inventory_movements`/`store_prices`/`shopping_list_items` before `inventory_items`/`shopping_lists`/`stores`/`products`) to avoid FK conflicts between tables purged in the same run, and wraps each table's delete in try/catch so one blocked table (e.g. a purge-eligible product still referenced by a live, non-deleted `inventory_items` row) doesn't abort the rest of the nightly job. BullMQ's `Queue`/`Worker` are given plain `ConnectionOptions` rather than a shared `ioredis` instance — passing an instance from this repo's top-level `ioredis` dependency fails to type-check against BullMQ's own bundled (differently-versioned) copy. Same live-verification gap as the rest of Phase 8: `tsc --noEmit` and `npm run build` are clean, no live Redis round trip against a running `docker compose` yet. **Merged** (GitHub PR #12).
> - **`feat/mobile-auth-sync`'s `HttpSyncTransport` reads the access token from `authStore` fresh on every call** rather than capturing it at construction, so a single instance (set once via `syncEngine.setTransport()` on sign-in) survives token rotation without needing to be recreated. On a 401 it calls `authStore.refreshAccessToken()` for one silent refresh-and-retry before giving up; a dead refresh token (expired/rotated elsewhere) drops the session via `authStore.logout()`. `authStore` deliberately does **not** import `HttpSyncTransport`/`syncEngine` (only `AuthClient` + `SecureTokenStore`) to avoid a store↔transport import cycle — the transport is wired up one-directionally from `app/_layout.tsx`'s new `AuthBootstrap` component, which also redirects a signed-out user out of `(tabs)` back to `(auth)/login`. The pre-existing splash screen (`app/index.tsx`) and the product tab's previously-inert "⋮" placeholder modal (`app/(tabs)/product.tsx`) were reused as the auth entry point and logout affordance respectively, rather than adding new UI chrome. `useNetworkStatus`'s exported shape (`{ isConnected, isInternetReachable }`) is unchanged — only the native branch's hardcoded `true` was swapped for real `@react-native-community/netinfo` listeners, so no caller needed touching. **Live-verification gap:** this sandbox's Metro/Expo web export fails on an unrelated pre-existing `react-native-worklets/plugin` resolution error (reproduced identically on unmodified `develop`, so not caused by this PR) — verified via `tsc --noEmit` (clean) only; a real device/emulator run against a live backend is still owed once tooling allows it.
> - **`feat/supabase-backend` (unmerged branch) replaces the local Postgres + bcrypt/self-signed-JWT/Redis-refresh-token stack above with a hosted Supabase project** — `AuthService` proxies `signUp`/`signInWithPassword`/`refreshSession`/admin `signOut` to Supabase Auth and verifies access tokens via Supabase's JWKS instead of a shared HS256 secret; `public.users` drops `password_hash` and FKs to Supabase's `auth.users` instead. `AuthTokens`/route shapes are unchanged, so mobile needed zero changes. Redis stays for `CleanupWorker` and the barcode cache only — it's no longer the refresh-token store. See docs/Supabase.md.
...
users           (id UUID PK, email UNIQUE, password_hash, created_at, updated_at)
                (on feat/supabase-backend: password_hash dropped, id FKs to Supabase's auth.users — see docs/Supabase.md)
